...
public final class MainActivity extends AppCompatActivity {
    ...
    private void chooseConsumableGroup() {
        ...
        new AlertDialog.Builder(this).setTitle(R.string.consumables_title).setItems(groups,(d,i)->{
            ...
            else if(i==1)editArray(R.string.catfruit_title,R.string.catfruit_label,document.catfruit(),document::setCatfruit);
    ...
    private void editTalentOrbs() { String[] actions=getResources().getStringArray(R.array.talent_orb_actions);new AlertDialog.Builder(this).setTitle(R.string.talent_orbs_title).setItems(actions,(d,i)->runFieldAction(()->{if(i==0)addTalentOrb();else if(document.talentOrbCount()==0)Toast.makeText(this,R.string.no_talent_orbs,Toast.LENGTH_SHORT).show();else chooseTalentOrb(i==2);})).setNegativeButton(R.string.close,null).show(); }
    private void chooseTalentOrb(boolean remove) { int count=document.talentOrbCount();String[] rows=new String[count];for(int i=0;i<count;i++)rows[i]=getString(R.string.talent_orb_row,document.talentOrbId(i),document.talentOrbAmount(i));new AlertDialog.Builder(this).setTitle(R.string.talent_orbs_title).setItems(rows,(d,i)->{if(remove){document.removeTalentOrb(i);persistApplied();}else editNumberText(rows[i],document.talentOrbAmount(i),v->document.setTalentOrbAmount(i,v));}).setNegativeButton(R.string.close,null).show(); }
    private void addTalentOrb() { LinearLayout form=new LinearLayout(this);form.setOrientation(LinearLayout.VERTICAL);form.setPadding(dp(24),dp(8),dp(24),0);EditText id=new EditText(this);id.setHint(R.string.talent_orb_id_label);id.setInputType(android.text.InputType.TYPE_CLASS_NUMBER);EditText amount=new EditText(this);amount.setHint(R.string.talent_orb_amount_label);amount.setInputType(android.text.InputType.TYPE_CLASS_NUMBER);form.addView(id);form.addView(amount);new AlertDialog.Builder(this).setTitle(R.string.talent_orb_add).setView(form).setNegativeButton(R.string.close,null).setPositiveButton(android.R.string.ok,(d,w)->{try{document.addTalentOrb(Integer.parseInt(id.getText().toString()),Integer.parseInt(amount.getText().toString()));persistApplied();}catch(Exception e){Toast.makeText(this,R.string.invalid_number,Toast.LENGTH_SHORT).show();}}).show(); }
    ...
    private void persistApplied() { workingCopy=document.toBytes();persistSession();Toast.makeText(this,R.string.edit_applied,Toast.LENGTH_SHORT).show(); }
