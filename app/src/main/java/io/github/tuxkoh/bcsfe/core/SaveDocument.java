...
public final class SaveDocument {
    ...
    public int catCount() { ensureCatProfile();if(gameVersion()<140300)return 873;int count=intAt(fixed(Offsets.offsets_38)-4);if(count<1||count>873)throw new IllegalStateException("Invalid cat count");return count; }
    ...
    public void removeTrueForms() { ensureCatProfile(); for(int i=0;i<catCount();i++){ putInt(fixed(Offsets.offsets_87)+i*4,0);putInt(fixed(Offsets.offsets_56)+i*4,0);int current=intAt(fixed(Offsets.offsets_55)+i*4);putInt(fixed(Offsets.offsets_55)+i*4,Math.min(current,1)); }touchRankUpSale();refreshHash(); }
    public void unlockFourthForms() { setFourthForms(false); }
    public void forceFourthForms() { setFourthForms(true); }
    private void setFourthForms(boolean force) { ensureCatProfile();for(int i=0;i<catCount();i++){int forms=GameDataRules.totalForms(i);if(force||forms>=4){unlockCatRaw(i);putFormValue(i,3);putInt(fixed(Offsets.offsets_55)+i*4,3);putFourthValue(i,2);}else if(forms>=3){unlockCatRaw(i);putFormValue(i,3);putInt(fixed(Offsets.offsets_55)+i*4,2);putFourthValue(i,0);}else if(forms==2){putFormValue(i,0);putInt(fixed(Offsets.offsets_55)+i*4,1);putFourthValue(i,0);}else{putFormValue(i,0);putInt(fixed(Offsets.offsets_55)+i*4,0);putFourthValue(i,0);}}touchRankUpSale();refreshHash(); }
    public void removeFourthForms() { ensureCatProfile(); for(int i=0;i<catCount();i++){ int current=intAt(fixed(Offsets.offsets_55)+i*4); putInt(fixed(Offsets.offsets_55)+i*4,Math.min(current,2)); putInt(fixed(Offsets.offsets_56)+i*4,0); }touchRankUpSale();refreshHash(); }
